...
console.log(
  `[worker:${workerName}] starting (queue=${WEBHOOK_CRITICAL_QUEUE_NAME}, concurrency=${workerConcurrency}, rps=${env.CRITICAL_LANE_RPS})`
);
...
export const webhookCriticalWorker = new Worker(
  ...
  async (job) => {
    ...
    console.log(
      `[worker:${workerName}] job active id=${job.id} attempt=${job.attemptsMade + 1} eventId=${eventId} subscriptionId=${subscriptionId}`
    );
  ...
  },
  {
    ...
    limiter: { max: env.CRITICAL_LANE_RPS, duration: 1000 },
  }
);
...
webhookCriticalWorker.on("failed", async (job, err) => {
  console.error(
